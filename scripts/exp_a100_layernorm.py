...
seeds_fewshot = [7004, 3639, 6290, 9428, 7056, 4864, 4273, 7632, 2689, 8219, 4523, 2175, 7356, 8975, 51, 4199, 4182, 1331, 2796, 6341, 7009, 1111, 1967, 1319, 741, 7740, 1335, 9991, 6924, 6595, 5358, 2638, 6227, 8384, 2769, 9933, 6339, 3112, 1349]
...
def run_exp(args):
    ...
    for hparams in itertools.product(dataset_vals,
        ...
        for format_n_shots_params in itertools.product(format_vals, n_shot_vals):
            ...
            if args.deepspeed:
                # cmd_prefix = "PYTHONPATH=. deepspeed input_to_label_and_rationale_a100_layernorm.py "
                cmd_prefix = "deepspeed scripts/input_to_label_and_rationale_a100_layernorm.py "
                ...
                # cmd_prefix = "PYTHONPATH=. python input_to_label_and_rationale_a100_layernorm.py "
                # cmd_prefix = "CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --debug scripts/input_to_label_and_rationale_a100_layernorm.py "
                # cmd_batch_size = f" --per_device_train_batch_size 1 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8 "

                cmd_prefix = "CUDA_VISIBLE_DEVICES=0 accelerate launch --debug scripts/input_to_label_and_rationale_a100_layernorm.py "
                cmd_batch_size = f" --per_device_train_batch_size 1 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8 "

            cmd = f'''{cmd_prefix} \
                    --output_dir {output_dir}  --model_type {model} --model_class {model}   \
                    --tokenizer_name {tokenizer_name}   --task_name {dataset}  --version_name v1.0 --do_train --dev_predict   \
                    ...
                    --max_steps {max_steps}  --lr_scheduler_type constant  --eval_steps {eval_steps}  --save_weight {save_weight_bool}'''
