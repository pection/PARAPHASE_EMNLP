...
def run_exp(args):
    ...
    for hparams in itertools.product(dataset_vals,
        ...
        for format_n_shots_params in itertools.product(format_vals, n_shot_vals):
            ...
            if args.deepspeed:
                # cmd_prefix = "PYTHONPATH=. deepspeed input_to_label_and_rationale_custom_loss_5.py "
                cmd_prefix = "deepspeed scripts/input_to_label_and_rationale_custom_loss_5.py "
                ...
                # cmd_prefix = "PYTHONPATH=. python input_to_label_and_rationale_custom_loss_5.py "
                # cmd_prefix = "CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --debug scripts/input_to_label_and_rationale_custom_loss_5.py "
                # cmd_batch_size = f" --per_device_train_batch_size 1 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8 "

                cmd_prefix = "CUDA_VISIBLE_DEVICES=0 accelerate launch --debug scripts/input_to_label_and_rationale_custom_loss_5.py "
                cmd_batch_size = f" --per_device_train_batch_size 1 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8 "
