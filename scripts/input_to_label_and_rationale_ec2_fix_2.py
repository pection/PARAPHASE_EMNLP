...
from metrics_ec2_fix_2 import evaluate
import torch
import datasets
import git
import time
...
def main():
    ...
    if data_args.generations_filepath is None:
        ...
        #     if "layer_norm" in name  or "SelfAttention.v" in name:
        ...
        peft_config = IA3Config(
                        peft_type="IA3",
                        task_type="SEQ_2_SEQ_LM",
                        target_modules=["k", "v", "wi_0"],
                        feedforward_modules=["wi_0"],
                    )
        print(f"IA3 CONFIG MODEL")
        model = get_peft_model(model, peft_config)
        model.print_trainable_parameters()
