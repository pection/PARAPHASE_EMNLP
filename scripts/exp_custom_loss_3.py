...
seeds_fewshot = [7004, 3639, 6290, 9428, 7056, 4864, 4273, 7632, 2689, 8219, 4523, 2175, 7356, 8975, 51, 4199, 4182, 1331, 2796, 6341, 7009]
seeds_fewshot = [7004,3638]
...
def run_exp(args):
    ...
    save_weight_bool = args.save_weight
    ...
    data_path_dict = {
        'esnli': None,
        'cos_e': None,
        'ecqa': '../data_2/ECQA-Dataset',
        'sensemaking': '../data_2/SenseMaking/',
        'sbic': '../data_2/SBIC/'
    }

    seed_map = {
        'esnli': [7004],
        'cos_e': [1111, 1181, 1319, 1331, 1335, 1349, 176, 178, 1936, 1967, 2051, 2147, 2175, 2348, 2498, 2638, 2689, 2769, 2796, 2913, 3112, 3639, 4182, 4199, 4273, 4512, 4523, 4779, 4791, 4823, 4864, 51, 5320, 5358, 588, 6227, 6290, 6339, 6341, 6595, 6895, 6924, 7004, 7009, 7056, 7356, 741, 7632, 7740, 8160, 8219, 834, 8384, 8483, 8975, 9428, 9599, 9933, 9962, 9991],
        'ecqa': [1181, 1349, 176, 178, 1936, 2051, 2147, 2348, 2498, 2769, 2913, 3112, 4512, 4779, 4791, 4823, 5320, 588, 6339, 6895, 8160, 834, 8483, 9599, 9933, 9962],
        'sbic': [1234],
        'sensemaking': [1331, 2175, 2689, 2796, 3639, 4182, 4199, 4273, 4523, 4864, 51, 6290, 6341, 7004, 7009, 7056, 7356, 7632, 8219, 8975, 9428]
    }
    ...
    for hparams in itertools.product(dataset_vals,
                                     ...
                                     tokenizer_vals):
        dataset, model, early_stopping_patience, max_steps, epochs, warmup_steps, learning_rate, per_device_train_batch_size, eval_steps, fewshot_eval_size, explanation_sep, tokenizer_name = hparams

        format_vals = format_dict[dataset]
        n_shot_vals = n_shots_dict[dataset]
        data_path = data_path_dict[dataset]

        for seed in seed_map.get(dataset, []):
            for format, n_shots in itertools.product(format_vals, n_shot_vals):

                run_name_parts = [dataset, str(seed), model, str(epochs), str(max_steps),
                                  str(warmup_steps), str(learning_rate), str(per_device_train_batch_size),
                                  str(eval_steps), explanation_sep, tokenizer_name, format, str(n_shots)]
                run_name = '-'.join(run_name_parts).replace('/', '-').replace('.', '').replace(' ', '')

                output_dir = os.path.join(args.exp_root, run_name)
                os.makedirs(output_dir, exist_ok=True)

                if args.deepspeed:
                    cmd_prefix = "deepspeed scripts/input_to_label_and_rationale_custom_loss_3.py "
                    ...
                    cmd_prefix = "CUDA_VISIBLE_DEVICES=0 accelerate launch --debug scripts/input_to_label_and_rationale_custom_loss_3.py "
                    cmd_batch_size = " --per_device_train_batch_size 1 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8 "

                cmd = f'''{cmd_prefix} \
                    --output_dir {output_dir} \
                    --model_type {model} --model_class {model} \
                    --tokenizer_name {tokenizer_name} \
                    --task_name {dataset} --version v1.0 \
                    --do_train --dev_predict \
                    --logging_first_step --logging_steps 1 \
                    --save_total_limit 1 --seed {seed} \
                    ...
                    --max_steps {max_steps} --lr_scheduler_type constant \
                    --eval_steps {eval_steps} --save_weight {save_weight_bool}'''

                wandb_env = f'WANDB_PROJECT={wandb_name} WANDB_NAME={run_name}'
                cmd = f"{wandb_env} {cmd}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--wandb_name", type=str, default='EMNLP', help="What is wandb project name")
    parser.add_argument("--save_weight", default=False, action='store_true', help="Save model weight")
